...
class Cart extends Front_Controller {
    ...
    function product($id)
    {
        ...
        $data['gift_cards_enabled'] = $this->gift_cards_enabled;
        $data['product_options'] = $this->_get_product_options($id);
	...
	function add_to_cart()
	{
		...
		/*
		  Disabling. We wont need to connect options with backend. We just stores it and mails it.
		...
		*/
            ...
            // lets name it product_options instead of options so it wont mess with gocart product options
			$product['options']	= $post_options;
...
//		} option validation disabled
	...
	***********************************************************/
    ...
    private function _get_product_options($id)
    {
        $this->config->load('product_options');
        $options = $this->config->item('product_options');
        // return default options if entry is missing
        if (! isset($options["$id"]))
        {
            $product_options = $options['default'];
        }
        elseif (isset($options["$id"]['remove']))
        {
            // remove the unwanted options
            $remove = $options["$id"]['remove'];
            unset($options["$id"]['remove']);
            
            $product_options = array_values(array_diff($options['default'], $remove));
            
            // verify there was no wrong entry
            $default_count = count($options['default']);
            $remove_count = count($remove);
            if (count($product_options) !== ($default_count - $remove_count))
            {
                show_error("Unrecognized value in remove ". var_export(array_diff($remove, $options['default']), true));
            }
        }
        else
        {
            $product_options = $options["$id"];
        }
