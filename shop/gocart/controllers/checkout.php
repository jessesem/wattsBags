	...
	*/
	...
	function place_order()
	{
        ...
        $cc = array(
            ...
            'CV2' => $this->input->post('CV2')
        );
        
        $cc['Validity'] = $this->input->post('StartDate_mm') . "/" . $this->input->post('StartDate_yy');
        $cc['Validity'] .= " to " . $this->input->post('ExpiryDate_mm') . "/" . $this->input->post('ExpiryDate_yy');
        ...
        $this->email->from(config_item('site_email'), config_item('site_company_name'));
        $this->email->to(config_item('site_email'));
        $this->email->subject("New Order #$order_id from {$data['customer']['firstname']} {$data['customer']['lastname']}");
        
        $order_primary_key = $this->order_model->primary_key_from_order_id($order_id);
        $data['order_primary_key'] = $order_primary_key;
        $admin_message = $this->load->view('order_email_admin', $data, true);
		...
		//email the admin
		$this->email->bcc(config_item('site_email'));
